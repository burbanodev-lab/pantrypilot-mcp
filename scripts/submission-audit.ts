...
const checks: Array<[string, boolean]> = [
  ...
  ['demo proves current kitchen_run source contract', /mealPlanSource/.test(demo) && /kitchenRunSource === ['"]unknown['"]/.test(demo)],
  ['demo proves durable household state', /hasPersistedDemoState/.test(demo) && /pantryCount\s*>?=\s*2/.test(demo) && /\.servings\s*===\s*2/.test(demo) && /mealPlanDays\.length\s*>?=\s*1/.test(demo)],
  ['demo failures propagate nonzero exit', /process\.exitCode\s*=\s*1/.test(demo) && !/process\.exit\(0\)/.test(demo)],
